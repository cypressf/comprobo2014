    ...
    for i in range(4):
        ...
        for j in range(4):
            ...
            for k in range(8):
                ...
                v = O[i + j * 4, k] / 10.0
                end_point = (
                    (i + 0.5) * V.shape[0] / 4.0 + v * cos(theta), (j + 0.5) * V.shape[0] / 4.0 - v * sin(theta))

                # compute the end points of the lines on the arrow head
                arrow_e1 = (
                    end_point[0] + cos(theta + 3 * pi / 4) * v / 4.0, end_point[1] - sin(theta + 3 * pi / 4) * v / 4.0)
                arrow_e2 = (
                    end_point[0] + cos(theta - 3 * pi / 4) * v / 4.0, end_point[1] - sin(theta - 3 * pi / 4) * v / 4.0)
...
def mouse_event(event, x, y, flag, dc):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        cv2.rectangle(im, (
            int(x / (visualizer_size[1] / patch_size[1])) - 1, int(y / (visualizer_size[0] / patch_size[0])) - 1),
                      (int(x / 4.0) + 1, int(y / 4.0) + 1), 1, -1)
